...
const UPDATE_PREDICTION_MUTATION = gql`
  mutation UpdatePredictionMutation($id: Int!, $input: UpdatePredictionInput!) {
...
export const Success = ({ prediction }: CellSuccessProps<EditPredictionById>) => {
  const [updatePrediction, { loading, error }] = useMutation(UPDATE_PREDICTION_MUTATION, {
    onCompleted: () => {
  ...
  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">Edit Prediction {prediction.id}</h2>
      </header>
      <div className="rw-segment-main">
        <PredictionForm prediction={prediction} onSave={onSave} error={error} loading={loading} />
      </div>
    </div>
  )
}
